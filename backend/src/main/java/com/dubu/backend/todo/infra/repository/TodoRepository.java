...
public interface TodoRepository extends JpaRepository<Todo, Long>{
    // todoId로 조회
    @Query("SELECT t FROM PastTodo t JOIN FETCH t.category c WHERE t.id = :todoId")
    Optional<Todo> findWithCategoryById(@Param("cursorTodoId") Long todoId);

    @Query("SELECT t FROM PastTodo t JOIN FETCH t.category WHERE t.id IN :todoIds")
    List<Todo> findTodosWithCategoryByIds(List<Long> todoIds);

    // 스케줄, 날짜로 조회
    @Query("SELECT t FROM PastTodo t JOIN FETCH t.category WHERE t.schedule = :schedule")
    List<Todo> findTodosWithCategoryBySchedule(@Param("schedule")Schedule schedule);

    @Query("SELECT t FROM PastTodo t JOIN t.schedule s WHERE t.parentTodo = :todo AND s.date = :date")
    Optional<Todo> findWithScheduleByParentTodoAndScheduleDate(@Param("todo") Todo parentTodo, @Param("date")LocalDate date);

    @Query("SELECT t FROM PastTodo t WHERE t.parentTodo = :parentTodo AND t.schedule = :schedule")
    Optional<Todo> findByParentTodoAndSchedule(@Param("parentTodo") Todo parentTodo, @Param("schedule") Schedule schedule);

    @Query("SELECT t.parentTodo.id FROM PastTodo t WHERE t.schedule = :schedule AND t.parentTodo IS NOT NULL")
    List<Long> findParentTodoIdsByScheduleAndParentTodoNotNull(Schedule schedule);

    // type 으로 조회
    @Query("SELECT t.parentTodo.id FROM PastTodo t WHERE t.parentTodo IN :parentTodos AND t.member = :member AND t.type = :type")
    ...
    @Query("SELECT t.parentTodo.id FROM PastTodo t WHERE t.id IN :ids AND t.type = :type AND t.parentTodo IS NOT NULL")
    List<Long> findParentTodoIdsByIdsAndTypeAndParentTodoNotNull(List<Long> ids, TodoType type);

    @Query("SELECT t From PastTodo t WHERE t.member = :member AND t.parentTodo = :parentTodo AND t.type = :type")
    ...
    // 경로로 조회
    @Query("SELECT t FROM PastTodo t WHERE t.subPath = :subPath")
    List<Todo> findTodosByPath(SubPath subPath);

    @Query("SELECT t FROM PastTodo t JOIN FETCH t.category WHERE t.subPath = :subPath")
    List<Todo> findTodosWithCategoryByPath(@Param("subPath") SubPath subPath);

    @Query("SELECT t.parentTodo.id FROM PastTodo t where t.subPath = :subPath AND t.parentTodo IS NOT NULL")
    List<Long> findParentTodoIdsByPathAndParentTodoNotNull(SubPath subPath);

    @Query("SELECT t FROM PastTodo t WHERE t.parentTodo = :parentTodo AND t.subPath = :subPath")
    Optional<Todo> findByParentTodoAndPath(@Param("parentTodo") Todo parentTodo, @Param("subPath") SubPath subPath);

    @Query("SELECT t FROM PastTodo t WHERE t.subPath.id IN :pathIds AND t.type = :type AND t.isCompleted = true")
    List<Todo> findByPathIdsAndTypeAndIsCompleted(List<Long> pathIds, TodoType type);

    // 계획으로 조회
    @Query("SELECT t FROM PastTodo t join fetch t.category join fetch t.subPath p WHERE p.plan = :plan AND t.isCompleted = :isCompleted")
